...
# MILA
# uesed by source memory -- same as test loader 800 x S image size 
def build_detection_train_memory_loader(cfg,  mapper=None):
    ...
    label_dataset = DatasetFromList(label_dicts)
    ...
    sampler = TrainingSampler(len(label_dataset))
    batch_sampler = torch.utils.data.sampler.BatchSampler(sampler, cfg.SOLVER.IMG_PER_BATCH_LABEL, drop_last=False) # 
    
    abc = torch.utils.data.DataLoader(
            label_dataset,
            num_workers=cfg.DATALOADER.NUM_WORKERS,
            batch_sampler=batch_sampler,
            collate_fn=trivial_batch_collator,  
    ...
    return abc, len(label_dataset)


   
# uesed by unbiased teacher trainer
def _data_len_(cfg, mapper=None):
    label_dicts = get_detection_dataset_dicts(
            cfg.DATASETS.TRAIN_LABEL,
            filter_empty=cfg.DATALOADER.FILTER_EMPTY_ANNOTATIONS,
            min_keypoints=cfg.MODEL.ROI_KEYPOINT_HEAD.MIN_KEYPOINTS_PER_IMAGE
