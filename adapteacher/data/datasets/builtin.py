...
from .sim10k import register_pascal_voc1
import io
import logging
...
#_root = os.getenv("DETECTRON2_DATASETS", "/lustre/home/70751243/conference_project/data/")
...
# ==== Predefined splits for sim10k (PASCAL VOC format) ===========
def register_all_sim10k(root):
    # root = "manifold://mobile_vision_dataset/tree/yujheli/dataset"
    SPLITS = [
        ("sim10k_train", "sim10k", "trainval10k"),
        ("sim10k_test", "sim10k", "trainval10k"),
    ]
    for name, dirname, split in SPLITS:
        year = 2012
        # register_pascal_voc(name, os.path.join(root, dirname), split, year, class_names=["person", "dog","bicycle", "bird", "car", "cat"])
        register_pascal_voc1(name, os.path.join(root, dirname), split, year)
        #MetadataCatalog.get(name).evaluator_type = "sim10k_city"
        #MetadataCatalog.get(name).thing_classes = ["person", "dog","bike", "bird", "car", "cat"]
        # MetadataCatalog.get(name).thing_classes = ["person", "dog","bicycle", "bird", "car", "cat"]
        # MetadataCatalog.get(name).evaluator_type = "coco"


# ==== Predefined splits for cityscapes (PASCAL VOC format) ===========
def register_all_city(root):
    root = "/lustre/home/70751243/conference_project/data"
    #SPLITS = [
    #    ("city_train", "cityscapes_in_voc", "trainval"),
    #    ("city_test", "cityscapes_in_voc", "test"),
    #]
    SPLITS = [
        ("city_train", "Cityscapes", "train"),
        ("city_test", "Cityscapes", "test"),
    ]
    for name, dirname, split in SPLITS:
        year = 2012
        # register_pascal_voc(name, os.path.join(root, dirname), split, year, class_names=["person", "dog","bicycle", "bird", "car", "cat"])
        #register_pascal_voc(name, os.path.join(root, dirname), split, year, class_names = ['bicycle','bus','car','motorcycle','person','rider','train','truck'])
        register_pascal_voc1(name, os.path.join(root, dirname), split, year)
        MetadataCatalog.get(name).evaluator_type = "sim10k_city"
        #MetadataCatalog.get(name).thing_classes = ["car"]
        # MetadataCatalog.get(name).thing_classes = ["person", "dog","bicycle", "bird", "car", "cat"]
        # MetadataCatalog.get(name).evaluator_type = "coco"


# ==== Predefined splits for Comic2k (PASCAL VOC format) ===========
def register_all_comic(root):
    ...
    for name, dirname, split in SPLITS:
        ...
        # register_pascal_voc(name, os.path.join(root, dirname), split, year, class_names=["person", "dog","bicycle", "bird", "car", "cat"])
        register_pascal_voc(name, os.path.join(root, dirname), split, year)
        MetadataCatalog.get(name).evaluator_type = "pascal_voc_comic"
        # MetadataCatalog.get(name).thing_classes = ["person", "dog","bike", "bird", "car", "cat"]
        # MetadataCatalog.get(name).thing_classes = ["person", "dog","bicycle", "bird", "car", "cat"]
        # MetadataCatalog.get(name).evaluator_type = "coco"
